    ...
    return {
      ...
      option: {
        ...
        column: [
          ...
          { label: '平台', prop: 'platformCode', search: true, editDisplay: false, editDisabled: true, hide: true, showColumn: false, type: 'select', value: 'ALI', change: (val) => { this.getRuleConfig(val) }, dicData: [{ label: 'ALIEXPRESS', value: 'ALI' }, { label: 'AMAZON', value: 'AMAZON' }, { label: 'DARAZ', value: 'daraz' }, { label: 'EBAY', value: 'EB' }, { label: 'JOOM', value: 'JM' }, { label: 'WISH', value: 'KF' }, { label: 'LAZADA', value: 'LAZADA' }, { label: 'MYMALL', value: 'MY' }, { label: 'SHOPEE', value: 'SHOPEE' }, { label: 'MEESHO', value: 'Meesho' }] },
          { label: '规则类型', prop: 'ruleType', editDisabled: true, search: true, rules: [{ required: true, message: '规则类型', trigger: 'blur' }], type: 'select', dicData: [{ value: 1, label: '所有平台指定SKU' }, { value: 2, label: '单个平台的所有规则指定SKU' }, { value: 3, label: '单个平台的所有规则指定账号' }, { value: 4, label: '单个平台的所有规则指定部门' }, { value: 5, label: '单个平台的所有规则指定商品编码' }, { value: 6, label: '单条规则指定SKU' }, { value: 7, label: '单条规则指定账号' }, { value: 8, label: '单条规则指定部门' }, { value: 9, label: '单条规则指定商品编码' }, { value: 10, label: '单条规则指定SKU指定账号' }, { value: 11, label: '单条规则指定SKU指定部门' }, { value: 12, label: '单条规则指定SKU指定商品编码' }, { value: 13, label: '单个平台的所有规则指定SKU指定账号' }, { value: 14, label: '单个平台的所有规则指定SKU指定部门' }, { value: 15, label: '单个平台的所有规则指定SKU指定商品编码' }] },
          ...
          { label: '排除类型值', prop: 'refVal', search: true, rules: [{ required: true, message: '排除类型值不能为空', trigger: 'blur' }] },
          ...
          { label: '级联类型值', prop: 'cascadeVal' },
          { label: '状态', prop: 'status', rules: [{ required: true, message: '状态不能为空', trigger: 'blur' }], type: 'select', dicData: [{ value: false, label: '无效' }, { value: true, label: '有效' }] }
        ]
      }
    }
  },
